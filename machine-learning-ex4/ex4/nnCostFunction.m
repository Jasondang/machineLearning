...
%z=Theta1 * X = X * Theta1' = [5000, 401] * [401, 25] = [5000, 25]
...
%z=Theta2 * a2 = a2 * Theta2' = [5000, 26] * [26, 10] = [5000, 10]
z3 = a2 * Theta2';  
a3 = sigmoid(z3);
...
J = b * sum(sum((-y.*log(h)) - ((1 - y).*(log(1-h)))));

c = lambda/(2*m);
reg = c * (sum(sum(Theta1(:,2:end).^2)) + sum(sum(Theta2(:,2:end).^2)));
